...
public class WeatherApiClientImpl implements WeatherApiClient {
  ...
  @Override
  public Optional<ForecastJsonResponse> getForecastWeather(
      String location, Integer days, Language lang) {
  ...
  public List<SearchJsonResponse> searchLocations(String query) {
    ...
    return get(URL_SEARCH, params, SearchJsonResponse[].class)
        .map(List::of)
        .orElse(Collections.emptyList());
  }

  private <T> Optional<T> get(
      String path, MultiValueMap<String, String> params, Class<T> responseType) {
  ...
  private String buildUrl(String path, MultiValueMap<String, String> params) {
    return UriComponentsBuilder.fromUriString(weatherApiProperties.getBaseUrl())
        .path(path)
        .queryParam(PARAM_API_KEY, weatherApiProperties.getApiKey())
        .queryParams(params)
        .build(false)
        .toUriString();
  }
