...
public final class TestUtils {
    ...
    void check_test_locations_and_names() throws IOException {
        List<ClassInfo> incorrectClass = getTestClasses().filter(c -> !findClass(StringUtils.substringBeforeLast(c.getName(), "Test")))
                .collect(Collectors.toList());

        assertTrue(incorrectClass.isEmpty(), "Following class are not located in the same package than their test class: "
    ...
    void test_must_not_use_SpringBootTest_annotation() throws IOException {
        List<Class<?>> incorrectClass = getTestClasses().map(ClassInfo::load).filter(test -> test.isAnnotationPresent(SpringBootTest.class))
                .collect(Collectors.toList());

        assertTrue(incorrectClass.isEmpty(), "Following test class are using the SpringBootTest annotation: "
